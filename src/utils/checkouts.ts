// X01 Checkout Suggestions
// Common checkout combinations for scores 2-170
...
// Checkout table for common scores (2-170)
// Only includes checkouts that are possible (ending on a double)
...
export function getCheckoutSuggestions(score: number, dartsRemaining: number = 3): string[][] {
    ...
    const suggestions = CHECKOUT_TABLE[score];
    if (!suggestions) {
        return [];
    }

    // Filter by number of darts remaining
    return suggestions.filter(combo => combo.length <= dartsRemaining);
}
